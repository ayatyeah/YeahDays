...
- **`POST /api/cron/lms-sync`** — тянет дедлайны из ical-экспорта календаря
  ...
  Когда прогон создаёт новые задачи, следом уходит ОДИН push на весь
  пакет (`sendPushToUser`, `kind: "day"`), а не по штуке на дедлайн:
  преподаватель выкладывает задания курса разом, и пять пушей подряд
  читаются как спам. `kind` именно `day`, а не `todo` — при `todo`
  service worker вешает кнопку «+10 мин» (`actionsFor` в `public/sw.js`),
  которой тут нечего откладывать. Тихие часы уважает `sendPushToUser` по
  локальному времени каждого устройства; если в этот момент тихо на всех,
  уведомление теряется — задачи при этом записаны, и человек увидит их в
  приложении.
