...
4. **Scoped-ключ внешнего сервиса** — `POST /api/integrations/*` и
   ...
   (`ApiKeyUser`, см. раздел 7). Ключ можно отозвать (`revokedAt`)
   независимо от остальных интеграций и от `ASSISTANT_SECRET`.
...
| `ApiKey` | ключ стороннего сервиса | `service, hash (bcrypt, unique), revokedAt?, lastUsedAt?` |
| `ApiKeyUser` | allowlist ключа — реально привязанные userId | `apiKeyId, userId (composite PK)` |
| `PairingCode` | одноразовый код привязки, TTL 10 мин | `code (PK), userId, expiresAt, consumedAt?` |
...
скриптом `scripts/create-api-key.mjs <service>` (сырой ключ печатается
один раз, дальше хранится только bcrypt-хэш).

- **`POST /api/keys/pair`** — сессия ИЛИ `userId` (пользователь генерирует
  код ДЛЯ СЕБЯ, в своём профиле YeahGrind). Тело `{ userId? }` → `{ code,
  expiresAt }`. Код — 8 символов, живёт 10 минут, одноразовый; новый код
  гасит старый непогашенный код того же пользователя.
- **`POST /api/keys/redeem`** — сервисный `ApiKey`. Тело `{ code }` →
  ...
  обмен уже использованного/просроченного кода → 400.
...
1. **Несколько независимых сервисов → scoped `ApiKey`** (раздел 6,
   ...
   (`scripts/create-api-key.mjs <service>`), сервис один раз просит
   пользователя вставить в свои настройки пейринг-код из профиля YeahGrind
   (`POST /api/keys/pair`, инициирует пользователь) и обменивает его на
   `userId` (`POST /api/keys/redeem`, инициирует сервис). Дальше сервис
   хранит этот `userId` у себя и вызывает `/api/integrations/*` тем же
   ключом. Ключ можно отозвать в любой момент независимо от остальных
   интеграций — просто `revokedAt` в `ApiKey`. Валидного ключа НЕ
   достаточно для доступа к произвольному пользователю — только к тем, кто
   реально прошёл обмен кода (`ApiKeyUser`); это не SSO (нет общего логина
   и редиректа), только доказательство владения аккаунтом в моменте.
