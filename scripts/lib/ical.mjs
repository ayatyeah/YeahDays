 ...
 *
 * Почему свой парсер, а не пакет с npm: нужно ~150 строк на разовый
 * импорт, а любой ical-пакет тянет зависимости в проект, где их и так
 * ровно столько, сколько нужно. Плюс здесь всё под тестами и видно, что
 * именно происходит с временными зонами — а это в дедлайнах главное.
 */
 ...
 * (например TZID="GMT+05:00"), поэтому просто indexOf(":") ошибётся.
 */
 ...
 */
export function zonedWallToUtc(y, mo, d, h, mi, s, timeZone) {
 ...
 */
export function parseDateValue(value, params, fallbackZone) {
 ...
 *
 * Возвращает сырые поля VEVENT, без какой-либо интерпретации под Moodle —
 * маппинг в задачи YeahGrind живёт отдельно (scripts/lib/moodle.mjs),
 * чтобы его можно было менять, не трогая разбор формата.
 */
export function parseEvents(icsText, fallbackZone = "UTC") {
  const events = [];
  let current = null;
