      ...
      {TABS.filter((t) => mounted.includes(t)).map((t) => {
        ...
        return (
          <div
            ...
            aria-hidden={!active}
            inert={!active}
            // "today" занимает всю ширину под свою сетку колонок (см.
            // TodaySection). Остальным даём широкую колонку по центру.
            //
            // Было max-w-2xl (672px) — из тех времён, когда разделы были
            // одноколоночными. С тех пор у календаря, профиля и «Сегодня»
            // появились двухколоночные lg-раскладки, и 672px их душили:
            // расписание сжималось до трети экрана, а по бокам оставались
            // две пустые полосы. 5xl даёт двум колонкам нормально встать.
            className={cn(
              "section-pane",
              exiting && "section-pane-exiting",
              t !== "today" && "lg:mx-auto lg:max-w-5xl lg:w-full",
            )}
          >
            <Section />
          </div>
